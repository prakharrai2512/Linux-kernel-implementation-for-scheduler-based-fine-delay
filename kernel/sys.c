...
SYSCALL_DEFINE1(test_print, char *, msg)
{
  char buf[256];
  long copied = strncpy_from_user(buf, msg, sizeof(buf));
  if (copied < 0 || copied == sizeof(buf))
    return -EFAULT;
  printk(KERN_INFO "test_print syscall called with \"%s\"\n", buf);
  return 0;
}


SYSCALL_DEFINE2(ultra_giga_maxduler, pid_t, pid_p,int,t_p)
{
  pid_t pid = pid_p;
  int t = t_p;
  int nice;
  struct task_struct *curt;
  curt = find_task_by_vpid(pid);
  nice = curt->static_prio;
  printk(KERN_INFO "ultra_giga_scheduler syscall called with \"%d\"\n", curt->static_prio);
  curt->custom_additional_latency_enabled = 1;
  curt->acdc.delay = t;
